...
```typescript
type Target = Element | (() => Element) | Ref<Element> | ComputedRef<Element>;
...
| options  | Setting                            | `Options` \| `Ref<Options>` \| `ComputedRef<Options>` \| `undefined` | -       |
...
| root       | The element that is used as the viewport for checking visibility of the target. Must be the ancestor of the target. Defaults to the browser viewport if not specified or if null. | `Element` \| `Document` \| `() => (Element/Document)` \|  `Ref<Element>` \| `ComputedRef<Element>` | -       |
