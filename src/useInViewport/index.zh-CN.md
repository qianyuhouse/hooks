...
| options | 设置                       | `Options` \| `Ref<Options>` \| `ComputedRef<Options>` \| `undefined` | -       |
...
| root       | 指定根(root)元素，用于检查目标的可见性。必须是目标元素的父级元素，如果未指定或者为 null，则默认为浏览器视窗。 | `Element` \| `Document` \| `() => (Element/Document)` \| `Ref<Element>` \| `ComputedRef<Element>` | -      |
